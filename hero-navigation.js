(function setupPortfolioVideoScenes(){
  const sceneConfig={
    ptd:{
      ...
      bgSelector:'.scene-bg',
      src:'https://www.pexels.com/download/video/14492116/',
      videoClass:'ptd-video-media'
    },
    skills:{
      section:document.getElementById('skills'),
      bgSelector:'.skills-video-bg',
      src:'https://www.pexels.com/download/video/15374243/',
      videoClass:'skills-video-media'
    }
  };

  function buildVideoScene(key,{section,bgSelector,src,videoClass}){
    if(!section)return;
    ...
    const markReady=()=>{
      ...
      section.classList.remove('video-error');
    };
    video.addEventListener('loadeddata',markReady);
    video.addEventListener('canplay',markReady);
    video.addEventListener('error',()=>{
      section.classList.remove('video-loaded');
  ...
  const oldStyle=document.getElementById('portfolio-video-scenes-css');
  if(oldStyle)oldStyle.remove();
  const style=document.createElement('style');
  style.id='portfolio-video-scenes-css';
  style.textContent=`
    #ptd.portfolio-video-scene,#skills.portfolio-video-scene{
      position:relative!important;overflow:hidden!important;isolation:isolate!important;
      background:#03070a!important;box-shadow:0 24px 60px rgba(15,23,42,.18)!important;
      padding:4.2rem 4.6rem!important;min-height:calc(100vh - 8.5rem)!important;
      border-radius:2rem!important;align-items:center!important;justify-content:center!important
    }
    #ptd.portfolio-video-scene.view-active,#skills.portfolio-video-scene.view-active{display:flex!important}
    #ptd .portfolio-video-bg,#skills .portfolio-video-bg{
      position:absolute!important;inset:0!important;z-index:0!important;width:100%!important;height:100%!important;
      overflow:hidden!important;border-radius:2rem!important;background:radial-gradient(circle at 65% 45%,#0a2530 0,#050b10 40%,#010203 82%)!important
    }
    #ptd .portfolio-video-media,#skills .portfolio-video-media{
      ...
      border:0!important;pointer-events:none!important;transform:scale(1.035)!important;
      filter:saturate(.92) contrast(1.08) brightness(.78)!important;background:#020305!important
    }
    #ptd .portfolio-video-bg::after,#skills .portfolio-video-bg::after{
      content:""!important;position:absolute!important;inset:0!important;z-index:2!important;pointer-events:none!important;
      background:linear-gradient(90deg,rgba(1,4,7,.48),rgba(1,4,7,.12) 50%,rgba(1,4,7,.46)),radial-gradient(circle at 50% 50%,transparent 0,rgba(0,0,0,.05) 55%,rgba(0,0,0,.38) 100%)!important
    }
    #ptd .section-scene-card,#skills .skills-video-card{
      position:relative!important;z-index:3!important;width:min(1120px,90%)!important;max-width:1120px!important;
      background:rgba(255,255,255,.96)!important;border:1px solid rgba(255,255,255,.84)!important;border-radius:2rem!important;
      padding:2.35rem 2.55rem!important;box-shadow:0 32px 78px rgba(0,0,0,.50),0 0 0 1px rgba(201,255,0,.07)!important;
      backdrop-filter:blur(4px)!important;-webkit-backdrop-filter:blur(4px)!important;animation:portfolioCardFloat 7s ease-in-out infinite!important
    }
    #skills .skill-card{background:rgba(249,250,251,.95)!important;box-shadow:0 10px 26px rgba(15,23,42,.07)!important}
    @keyframes portfolioCardFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}
    ...
    @media(max-width:480px){
      ...
      #ptd .portfolio-video-media,#skills .portfolio-video-media{filter:saturate(.86) contrast(1.05) brightness(.72)!important}
    }
    @media(prefers-reduced-motion:reduce){#ptd .section-scene-card,#skills .skills-video-card{animation:none!important}}
  `;
